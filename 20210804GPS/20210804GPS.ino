...
void setup()
{
  ...
  myGNSS.setI2COutput(0x42, defaultMaxWait); //Set the I2C port to output UBX only (turn off NMEA noise)
  myGNSS.saveConfigSelective(VAL_CFG_SUBSEC_IOPORT, defaultMaxWait); //Save (only) the communications port settings to flash and BBR
 
  boolean response = myGNSS.enableSurveyMode(300, 2.000);
