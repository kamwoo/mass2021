...
class massMathMotorCalculateAngle:
    ...
    ## 실험을 통해서 구한 상수들
    strongStraight = [110, 110]
    weakStraight = [105, 105]
    right = [104, 77]
    left = [77, 104]
    turnDistance = 1.0  ## 1m
    changeStraightDistance = 5.0
    ...
    def CalculateFinalMotorValue(self, tempDiffrenceAngle, tempRemainDistance):
        motor = [90,90]        
        if(tempRemainDistance > self.changeStraightDistance):
            motor[0] = self.strongStraight[0]
            motor[1] = self.strongStraight[1]
        else:
            motor[0] = self.weakStraight[0]
            motor[1] = self.weakStraight[1]
    ...
    def RealWhileAllMethod(self, tempGpsPoint, tempCompassAngle):
        ...
        motorValue = self.CalculateFinalMotorValue(diffrenceAngle, remainDistance)
