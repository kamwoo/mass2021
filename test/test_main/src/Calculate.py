    ...
    def CalculateFinalMotorValue(self, motorSpeed, angleOfBaseExtendeLine,
        ...
        if (angleOfBaseYcordinate != 0.0):
            if (angleOfBaseExtendeLine>=0.0):
                motor[0] = motorSpeed * (inverseSpeed + 1.0 * (abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate) + tempCorrectionFactor))
                motor[1] = motorSpeed * (inverseSpeed - 1.0 * (abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate) + tempCorrectionFactor))
            else:
                motor[0] = motorSpeed * (inverseSpeed - 1.0 * (abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate) + tempCorrectionFactor))
                motor[1] = motorSpeed * (inverseSpeed + 1.0 * (abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate) + tempCorrectionFactor))
