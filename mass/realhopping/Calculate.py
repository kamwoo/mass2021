...
class CalculateClass :
    ...
    def __init__(self, startButtonOnAndOff, objectAllLists):
        ## Define Global Variable
        ## 전역변수를 정의한다.
        self.startButtonOnAndOff = startButtonOnAndOff ## 1로 변환되면 while문이 돌게 된다.
    ...
    def CalculateFinalMotorValue(self, motorSpeed, angleOfBaseExtendeLine,
        ...
        if (angleOfBaseYcordinate != 0.0):
            if (angleOfBaseExtendeLine>=0.0):
                motor[0] = motorSpeed * (1.0 + 1.0 * abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate))
                motor[1] = motorSpeed * (1.0 - 1.0 * abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate))
            else:
                motor[0] = motorSpeed * (1.0 - 1.0 * abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate))
                motor[1] = motorSpeed * (1.0 + 1.0 * abs(angleOfFromNowPositionAndObject / angleOfBaseYcordinate))
