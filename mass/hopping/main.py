...
def main():
    ...
    extend_math_li_trash = direction_list ##바라보는 방향
    ...
    while(True):
        ...
        # ROS에서 [위도, 경도, heading angle]
        p_real_time = [x,y]

        angle_math_li_trash = p_real_time##IMU들어가서 Heading angle이 들어오게 된다면 이것은 필요없다
...
def temporary_heading_angle(jjj):##그냥 임시로 확인하기 위한 함수이다. IMU로 heading angle을 가져온다면 이 함수는 없애주자
    heading_angle = []
...
def automatic_angle_distance (p_real_time, extend_math_trash, y_math_trash):
    ...
    want_distance = point_to_point_distance(p_real_time, extend_math_trash[2])
    RR = max_speed_select(p_real_time)
