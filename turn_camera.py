...
# 'http://192.168.0.217:8080/shot.jpg'
url = 2



# YOLO 가중치 파일과 CFG 파일 로드
YOLO_net = cv2.dnn.readNet("yolov3_final2.weights", "yolov3.cfg")
...
while (VideoSignal.isOpened()):
    ...
    indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.45, 0.4)  # boxes 많은것 제거
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            try:
                ...
                print(
                    'IndexError IndexError IndexError IndexError IndexError IndexError IndexError IndexError IndexError IndexError')

            # 경계상자와 클래스 정보 이미지에 입력
            if label == classes[0]:
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 1)  # 네모 외각만 1, 네모 색칠 -1
    ...
    #hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)  # 색상 공간 변환
    rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

    #lower_orange = np.array([1, 100, 100])  # 주황색
    #upper_orange = np.array([40, 255, 250])

    #lower_white = np.array([0, 0, 180])  # 흰색
    #upper_white = np.array([360, 30, 255])

    lower_green = np.array([0, 255, 0])  # 초록색 (Yolo 검출)
    upper_green = np.array([0, 255, 0])

    #mask_orange = cv2.inRange(hsv, lower_orange, upper_orange)  # 주황색 범위 add
    #mask_white = cv2.inRange(hsv, lower_white, upper_white)  # 흰색 범위 add
    mask_green = cv2.inRange(rgb, lower_green, upper_green)  # 초록색 범위 add

    #mask_fir = cv2.addWeighted(mask_orange, 1.0, mask_white, 1.0, 0.0)  # 주황 + 흰
    #mask = cv2.addWeighted(mask_fir, 1.0, mask_green, 1.0, 0.0)  # mask_fir + 초록
    ...
    # cv2.findContours(imgae, mode, method)
    # mode : cv2.RETR_EXTERNAL , cv2.RETR_LIST, cv2.RETR_CCOMP , cv2.RETR_TREE
    # method : cv2.CHAIN_APPROX_NONE , cv2.CHAIN_APPROX_SIMPLE , cv2.CHAIN_APPROX_TC89_L1 , cv2.CHAIN_APPROX_TC89_KCOS
    ...
    for cont in contours:
        ...
        if cv2.contourArea(cont) > 90000.0:  # 숫자 바꾸기
    ...
    blur = cv2.bilateralFilter(res1, 9, 40, 40)  # Blurring the image to remove the noise present in the image.
    edges = cv2.Canny(blur, 50, 100)  # 가장자리 검출
    ...
    blurred_frame = cv2.bilateralFilter(res1, 9, 75, 75)  # 노이즈 제거
    gray = cv2.cvtColor(blurred_frame, cv2.COLOR_BGR2GRAY)  # gray 스케일 변환
    ret, thresh = cv2.threshold(gray, 106, 255, 1)  # 그림자 제거
    ...
    if forwardEdge[0] > 50:  # 여기 바꿔
        if farthest_point[1] < 310:  # Checking for the farthest_point on the left of the frame.
            direction = "Move left"  # forwardEdge y좌표 250보다 크고, farthest_point x좌표 310보다 작으면
            print(direction)
        else:
            direction = "Move right"  # forwardEdge y좌표 250보다 크고, farthest_point x좌표 310보다 크면
            print(direction)
    else:
        direction = "Move forward"  # forwardEdge y좌표 250보다 작으면 직진
        print(direction)
